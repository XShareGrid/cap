...
func generateTranslator(workdir string, packageName string) {
	...
	f, err := os.OpenFile(translatorFileName, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
	...
	}{packageName, template.HTML(time.Now().String())})
	if err != nil {
		os.Remove(translatorFileName)
...
func generateConstants(workdir string, packageName string) {
	...
	for _, k := range idArray {
		f.Comment(k + " " + idMap[k])
		f.Const().Id(k).Op("=").Qual("github.com/XShareGrid/cap/msg/i18n", "TrID").Call(jen.Lit("v_" + packageName + "_" + k))
	}
...
func generateLangFromToml(workdir, packageName string, tf os.FileInfo) {
	...
	f := jen.NewFile(packageName)
	f.PackageComment(fmt.Sprintf("Code generated by cap from [%s] at [%s], DO NOT EDIT.", fileName, time.Now().String()))
